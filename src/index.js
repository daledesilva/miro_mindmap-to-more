...
async function getMindMap() {
    let selectedWidgets = await miro.board.get();//getSelection();
...
function getRootNode(nodes) {
    ...
    for(let k=0; k<nodes.length; k++) {
      console.log(`node ${k}:`,nodes[k]);
      if(nodes[k].type === "mindmap") {
            return nodes[k];
      }
...
function getChildNodeTreesFrom(nodesLeft, parentNode) {
    ...
    for(let k=0; k<nodesLeft.length; k++ ) {
